...
let rec typecheck(m: ctx) (e: term): myType err =
  ...
  | IsEq(e1,e2)     -> let te1 = typecheck m e1 in
                       let te2 = typecheck m e2 in
                       if te1 = te2 then Yes TBool else No "ill-typing in iseq"
  ...
  | PLambda(x,e1)   -> let m' = extend m x (TVar x) in
                       let te1 = typecheck m' e1 in
                       (
                        match te1 with
                        | Yes t1 -> Yes(Forall(x,t1))
                        | _      -> No "ill-typing in plambda" 
                       )
  | TApp(e1,t1)     -> let te1 = typecheck m e1 in
                       (
                        match te1 with
                        | Yes(Forall(x,t1')) -> Yes(substType t1' x t1)
                        | _                  -> No "ill-typing in tapp"
                       )
  | Cons(e1,e2)     -> let te1 = typecheck m e1 in
                       let te2 = typecheck m e2 in
                       (
                        match (te1,te2) with
                        | (Yes t1, Yes(List t2)) when t1 = t2 -> Yes(List t2)
                        | _                                   -> No "ill-typing in cons"
                       )
  | Nil t1           -> Yes(List t1) 
  | Head e1          -> let te1 = typecheck m e1 in
                        (
                         match te1 with
                         | Yes(List t1) -> Yes t1
                         | _             -> No "ill-typing in head"
                        )
  | Tail e1          -> let te1 = typecheck m e1 in
                        (
                         match te1 with
                         | Yes(List t1) -> Yes(List t1)
                         | _             -> No "ill-typing in tail"
                        )